    ...
    ($forward_name:ident, $reduce_fn_name:ident, $reduce_default:ident) => {
        fn $forward_name<'v, T: Float>(
            ...
            if ndarray_ext::is_scalar_shape(x_shape) {
                ...
                crate::OpOutput::Owned(folded.unwrap_or_else(|| x.to_owned()))
            }
        }
    };
}
...
impl<T: Float> op::Op<T> for ReduceMean {
    fn compute(&self, ctx: &mut crate::op::ComputeContext<T>) -> Result<(), crate::op::OpError> {
        ...
        let x_shape = x.shape();
        if axes.is_empty() {
            ctx.append_output_view(x.clone());
            return Ok(());
        }
...
fn min_max_grad<'g, T: Float>(
    ...
    let gy = Tensor::builder(ctx.graph())
        ...
        .build(grad_op2);
    let eq = equal(x1, y);
    ctx.append_input_grad(Some(mul(eq, gy)));
    ctx.append_input_grad(None);
}
